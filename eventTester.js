...
function writeCarTelemetry(buf, offset, values = {}) {
    const v = {
        ...
        tyresSurfaceTemperature: [80, 80, 80, 80], // default: cool, below threshold
...
function writeCarStatus(buf, offset, values = {}) {
    const v = {
        ...
        fuelInTank: 45.2, fuelCapacity: 110, fuelRemainingLaps: 8.4, // default: plenty of fuel
...
function buildCarStatusPacket(overrides = {}) {
    const totalSize = HEADER_SIZE + CAR_STATUS_SIZE * NUM_CARS;
    const buf = Buffer.alloc(totalSize);
    let offset = writeHeader(buf, 0, 7);
    offset = writeCarStatus(buf, offset, overrides);
    for (let i = 1; i < NUM_CARS; i++) offset = writeCarStatus(buf, offset);
    return buf;
}

function buildLapDataPacket(overrides = {}) {
    const totalSize = HEADER_SIZE + LAP_DATA_SIZE * NUM_CARS;
    const buf = Buffer.alloc(totalSize);
    let offset = writeHeader(buf, 0, 2);
    offset = writeLapData(buf, offset, overrides);
    for (let i = 1; i < NUM_CARS; i++) offset = writeLapData(buf, offset);
    return buf;
}

function sendAll(packets) {
    const socket = dgram.createSocket('udp4');
    let sent = 0;
    for (const packet of packets) {
        socket.send(packet, 20777, '127.0.0.1', () => {
            sent += 1;
            if (sent === packets.length) socket.close();
        });
    }
}

async function run() {
    console.log('Sending round 1: normal state (cool tyres, plenty of fuel, P5)...');
    sendAll([
        buildCarTelemetryPacket({ tyresSurfaceTemperature: [80, 80, 80, 80] }),
        buildCarStatusPacket({ fuelRemainingLaps: 8.4 }),
        buildLapDataPacket({ carPosition: 5, currentLapNum: 12 }),
    ]);

    await new Promise((resolve) => setTimeout(resolve, 1500));

    console.log('Sending round 2: front tyres now hot, fuel now low, gained a position...');
    sendAll([
        buildCarTelemetryPacket({ tyresSurfaceTemperature: [80, 80, 110, 112] }), // FL/FR now hot
        buildCarStatusPacket({ fuelRemainingLaps: 1.5 }), // below LOW_FUEL_LAPS threshold
        buildLapDataPacket({ carPosition: 4, currentLapNum: 12 }), // P5 -> P4
    ]);

    console.log('Done sending test sequence.');
}
